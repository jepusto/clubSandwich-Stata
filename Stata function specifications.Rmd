...
relevant_coefs <- !grepl("factor\\(state\\)", names(coef(wls_within)))
coef_test(wls_within, vcov = "CR2", 
          cluster = MV_Mortality$state, 
          inverse_var = TRUE)[relevant_coefs, ]

# p-weighted
coef_test(wls_within, vcov = "CR2", 
          cluster = MV_Mortality$state, 
          inverse_var = FALSE, ignore_FE = TRUE)[relevant_coefs, ]
